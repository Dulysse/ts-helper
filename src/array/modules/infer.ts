 ...
 */
export declare const arrayError: unique symbol;
 ...
 * type B = Arr.Infer<[1, 2, 3], { minLength: 4 }>; // { arrayError: "The array (3 elements) must contain at least 4 elements." }
 * type C = Arr.Infer<[1, 1, 3], { unique: true }>; // { arrayError: "The array must have only unique elements." }
 * ```
 * ---------------------------
	...
	Not<IsTuple<TArray>> extends true
		...
		: Rules["minLength"] extends number
			? CheckMinLength<TArray, Rules["minLength"]> extends false
				? {
						[arrayError]?: `The array (${Length<TArray>} elements) must contain at least ${Rules["minLength"]} elements.`;
					}
				: Infer<TArray, Omit<Rules, "minLength">>
			: Rules["maxLength"] extends number
				? CheckMaxLength<TArray, Rules["maxLength"]> extends false
					? {
							[arrayError]?: `The array (${Length<TArray>} elements) must contain a maximum of ${Rules["maxLength"]} elements.`;
						}
					: Infer<TArray, Omit<Rules, "maxLength">>
				: Rules["unique"] extends true
					? Equal<TArray, Unique<TArray>> extends false
						? {
								[arrayError]?: `The array must have only unique elements.`;
							}
