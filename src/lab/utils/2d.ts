...
/**
 * The row limit for the game board.
 * - This is used to ensure that the game board does not exceed a certain number of rows.
 * - It is set to 20 by default, but can be adjusted as needed.
 */
declare type ROW_LIMIT = 20;

/**
 * The column limit for the game board.
 * - This is used to ensure that the game board does not exceed a certain number of columns.
 * - It is set to 20 by default, but can be adjusted as needed.
 */
declare type COL_LIMIT = 20;
 ...
 */
declare type Cell = GameRules["PLAYERS"][number] | GameRules["EMPTY_CELL"];
...
export declare type IsBoardFull<
	...
	TRules extends GameRules,
> = TBoard extends readonly Cell[][]
	? TRules["EMPTY_CELL"] extends TBoard[number][number]
		? false
		: true
	: false;
 ...
 */
export declare type Infer2DGameErrors<
	TBoard extends Cell[][],
	TRules extends GameRules,
 ...
 */
declare type InferRulesError<TRules extends GameRules> =
	Or<
	...
	> extends true
		...
		: Or<
					...
					>
			  > extends true
			? " ❌ Invalid game rules! The rules must not be native types. "
			: Or<IsNegative<TRules["ROW"]>, IsFloat<TRules["ROW"]>> extends true
				? " ❌ Invalid game rules! The number of rows must be a positive integer. "
				: Or<IsNegative<TRules["COL"]>, IsFloat<TRules["COL"]>> extends true
					? " ❌ Invalid game rules! The number of columns must be a positive integer. "
					: Or<
								IsNegative<TRules["TARGET_SCORE"]>,
								IsFloat<TRules["TARGET_SCORE"]>
						  > extends true
						? " ❌ Invalid game rules! The target score must be a positive integer. "
						: TRules["PLAYERS"] extends [
							...
							? Or<
								...
								: Or<
									...
									: Greater<TRules["TARGET_SCORE"], TRules["ROW"]> extends true
										? " ❌ Invalid game rules! The target score must be less than or equal to the number of rows. "
										: Greater<
													TRules["TARGET_SCORE"],
													TRules["COL"]
											  > extends true
											? " ❌ Invalid game rules! The target score must be less than or equal to the number of columns. "
											: Greater<TRules["COL"], COL_LIMIT> extends true
												? ` ❌ Invalid game rules! The number of columns must be less than or equal to ${COL_LIMIT}. `
												: Greater<TRules["ROW"], ROW_LIMIT> extends true
													? ` ❌ Invalid game rules! The number of rows must be less than or equal to ${ROW_LIMIT}. `
													: null
